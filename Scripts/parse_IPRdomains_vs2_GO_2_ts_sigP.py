#!/usr/bin/python
#Written by AnjaSpang, February 2017
#Edited by Nina Dombrowski to look for additional features
#for python3.5
...
#Read genes with corresponding TMHMM domains into dictionary
##########################################################################################

def TMHMMdomain_dictionary(IPRscan_out):
        TMHMMdomains = {}
        ...
        for line in IPRscan_out:
                try:
                        entry = line.split('\t')
                        if 'TMh' in entry[4]:
                                if entry[0] not in TMHMMdomains:
                                        TMHMMdomains[entry[0]] = []
                                        TMHMMdomains[entry[0]].append(entry[4])
                                else:
                                        if entry[4] not in TMHMMdomains[entry[0]]:
                                                TMHMMdomains[entry[0]].append(entry[4])
                except:
                        pass
...
#Read genes with corresponding SignalP annotations into dictionary
##########################################################################################

def SigPdomain_dictionary(IPRscan_out):
        SigPdomains = {}
        ...
        for line in IPRscan_out:
                try:
                        entry = line.split('\t')
                        if 'Sig' in entry[4]:
                                if entry[0] not in SigPdomains:
                                        SigPdomains[entry[0]] = []
                                        SigPdomains[entry[0]].append(entry[4].strip())
                                else:
                                        if entry[4] not in SigPdomains[entry[0]]:
                                                SigPdomains[entry[0]].append(entry[4].strip())
                except:
                        pass
...
#GeneID	IPRdomain	IPRdescription	PFAMdomain	PFAMdescription	TMHMMdomains	GOresults KEGGresults
##########################################################################################
...
TMHMMdomains = TMHMMdomain_dictionary(infile)
SigPdomains = SigPdomain_dictionary(infile)
...
for geneID in geneIDs:
	...
	if geneID in PFAMdescr:
        ...
        if geneID in TMHMMdomains:
                    TMHMMdomain = '; '.join(TMHMMdomains[geneID])
        else:
                    TMHMMdomain = "-"

        if geneID in SigPdomains:
                    SigPdomain = '; '.join(SigPdomains[geneID])
        else:
                    SigPdomain = "-"


	outfile.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (geneID, IPRdomain, IPRdes, PFAMdomain, PFAMdes, GOnumber, ECnumber, TMHMMdomain, SigPdomain))
